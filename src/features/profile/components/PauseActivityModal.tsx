...
export function PauseActivityModal({
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to pause activity',
        variant: 'destructive',
      });
    }
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="space-y-6 py-4">
          <RadioGroup value={durationType} onValueChange={(value) => {
            setDurationType(value as 'ONE_PERIOD' | 'N_PERIODS' | 'UNTIL_DATE' | 'INDEFINITE');
